...
func runHook(root string, in hookInput) (map[string]any, error) {
	...
	context := "Tincan has pending events for this task: " + string(data) + ". For connection_notice: " + lifecycleInstructions + " For join_requested: " + joinReviewInstructions + " For approval_needed or needs_attention: read inbox_requests, surface its concrete question to this user and record delivery with inbox_decide; never treat this notice as approval. For collaboration_attention: " + inboundDispatchInstructions + " " + collaborationReviewInstructions + " For messages (prioritize direct mentions): " + inboundDispatchInstructions
	if in.Event == "Stop" {
