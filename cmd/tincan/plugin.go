...
Claude native channel events have kind paired, message or mention. A paired event is a protocol receipt: report the peer name briefly if useful; do not send another acknowledgement. A message or mention includes connection and event_seq for dispatch, without the peer body. Delegate it in the background as described in the inbound instructions. The worker claims and retrieves the body, handles authorized work, then uses inbox_reply(connection,seq,text,claim) or inbox_ack(connection,seq,claim). Do not acknowledge unfinished work. Pull broader context with messages_search. Respond to eligible peer messages by default, including messages without mentions. Prioritize direct mentions when selecting pending work and relevant context. Self messages and automated replies do not wake the model. Without native host support, messages stay queued and inbox_next retrieves the current pending item immediately; do not busy-poll.
