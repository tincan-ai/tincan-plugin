...
func (b *pluginBroker) serverWithTools(remoteTools []*mcp.Tool) *mcp.Server {
	...
	type connectInput struct {
		...
		Profile                 string              `json:"profile,omitempty" jsonschema:"Public profile (up to 2000 characters): include on fresh connections unless the user declines. Briefly describe your owner or principal, collaboration role and actual capabilities. Use an identity provided for sharing or ask who to name and update after they answer; respect anonymity and exclude private project details."`
	...
	for _, original := range remoteTools {
		...
		if tool.Name == "room_create" {
			props["e2ee"] = map[string]any{"type": "boolean", "description": "Default false. Explicitly opt this new room into E2EE on Party Line or Switchboard. Uses local keys; other rooms keep their modes."}
		}
